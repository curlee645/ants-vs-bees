...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while place is not beehive:
            if len(place.bees)>0 and self.min_range<=place_counter<=self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting>0:
            ...
            bee_to_be_eaten=rANTdom_else_none(self.place.bees)
            if bee_to_be_eaten==None:
                pass
            else:
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.num_queens=QueenAnt.num_queens+1
    ...
    def action(self, gamestate):
        ...
        if self.true_Queen==False:
            ...
            ThrowerAnt.action(self,gamestate) #might be error; check
            place=self.place.exit
            while place is not None:
                if place.ant is not None and place.ant.damage_doubled==False:
                    place.ant.damage_doubled=True
                    place.ant.damage=place.ant.damage*2
                place=place.exit
    
            #1.go through the places
            #2.if the place has an ant 
            # and the ant's damage_doubled is False
            # make the ant's damaged_double True and double damage
            #else next place

        #if imposter, reduce armor to 0
        #throw a leaf
        #double the damage of ants behind
    ...
    def reduce_armor(self, amount):
        ...
        if self.true_Queen:
            if self.armor<=0:
                bees_win()
        #take into account imposter queen
